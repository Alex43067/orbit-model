...
r0 = best_kid[0:3]
rdot0 = best_kid[3:6]
std = np.append(3*np.abs(r0), np.abs(rdot0))

var = 1.01
pop = 20

pos_list = integrate(t_init, t_end, dt, r0, rdot0, t_list)
...
# pos_list = integrate(t_init, t_end, dt, r0, rdot0, t_list)
# ax.plot3D(pos_list[:,0], pos_list[:,1], pos_list[:,2])
# plt.show()

# if one of the values is close to nasa value, it will be hard to progress because that value will probably be changed,
# which will cause the error to increase